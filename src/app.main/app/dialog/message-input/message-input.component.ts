...
import { Component, OnInit, OnDestroy, AfterViewInit, ChangeDetectorRef, ChangeDetectionStrategy, ViewChild, ElementRef, Renderer, Input, Output, EventEmitter } from '@angular/core';
...
export class MessageInputComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit() {
        ...
        this.restoreCachedMessages(this.peerId, this.isChat);
        ...
        this.removeAttachment.subscribe(value => {
            ...
            const i = this.attachments.findIndex(x => x.id === value);
            if (i > -1) {
    ...
    restoreCachedMessages(id, isChat) {
        this.store.select(s => s.inputMessages)
            .map(im => im.messages[this.peerId])
            .first()
            .subscribe(message => {
                if (message) {
    ...
    cacheCurrentMessage() {
        this.messagesService.typeMessage(this.peerId, { body: this.inputText, attachments: this.attachments }, this.isChat);
    }
    ...
    sendMessage(text: string) {
        ...
        this.messagesService.sendMessage(this.peerId, { body: text, attachments: this.attachments }, this.isChat);
