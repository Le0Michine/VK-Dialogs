        ...
        trigger('toggleEmoji', [
            ...
            transition('out => in', [
                animate(200, keyframes([
                    ...
                    style({opacity: 1, height: '210px', offset: 1.0})
                ]))
            ]),
            transition('in => out', [
                animate(200, keyframes([
                    style({opacity: 1, height: '210px', offset: 0}),
...
export class EmojiComponent implements OnInit {
    ...
    ngOnInit() {
        const emojiWrapper = document.getElementById('emoji_wrapper');
        ...
        const emojiImgs = document.getElementsByClassName('emoji');
        for (let i = 0; i < emojiImgs.length; i++) {
            const emojiImg = emojiImgs.item(i) as HTMLImageElement;
            if (emojiImg) {
                emojiImg.onclick = () => this.selectEmoji(emojiImg.alt);
            }
        }
        const flagsImg = document.getElementById('flags').getElementsByTagName('img');
    ...
    selectTab(page: string) {
        const currentPage = this.states.findIndex(x => x.state === 'in');
        const newPage = this.states.findIndex(x => x.page === page);
