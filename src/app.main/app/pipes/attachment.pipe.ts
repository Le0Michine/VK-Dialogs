...
export class MessageAttachmentSubTitlePipe implements PipeTransform {
    constructor(private translate: TranslateService) { }

    transform(attachment) {
        const tmp = attachment.doc || attachment.audio || attachment.video || attachment.wall || attachment.link || {size: -1};
        if (attachment.doc) {
            ...
            const min = Math.floor(tmp.duration / 60);
            const sec = tmp.duration % 60;
...
export class MessageAttachmentUrlPipe implements PipeTransform {
    transform(attachment, uid = undefined) {
        const tmp = attachment.doc || attachment.audio || attachment.video || attachment.wall || attachment.link;
        if (attachment.doc || attachment.audio || attachment.link) {
            ...
            const ll = attachment.geo.coordinates.split(' ').join(',');
...
export class MessageAttachmentTitlePipe implements PipeTransform {
    transform(attachment) {
        const tmp = attachment.doc || attachment.audio || attachment.video || attachment.wall || attachment.link || attachment.geo;
...
export class MessageAttachmentIconPipe implements PipeTransform {
    transform(atttachment) {
        if (atttachment.video) {
            return 'att_vid_img';
        }
        if (atttachment.doc && atttachment.doc.ext === 'zip') {
            return 'att_zip_img';
        }
        if (atttachment.doc || atttachment.wall) {
            return 'att_doc_img';
        }
        if (atttachment.audio) {
            return 'att_aud_img';
        }
        if (atttachment.geo) {
            return 'att_doc_img';
