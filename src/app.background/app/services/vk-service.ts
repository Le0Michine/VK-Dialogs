...
import { Observable, ConnectableObservable, Subject, BehaviorSubject } from 'rxjs/Rx';
...
export class VKService {
    ...
    auth(force: boolean = false): Observable<SessionInfo> {
        ...
        if (!this.isSessionValid()) {
            const obs = AuthHelper.authorize(force);
            obs.catch(error => {
    ...
    setOnline(): void {
        this.getSession().concatMap(session => {
            ...
            const uri: string = VKConsts.apiUrl + this.setOnlineApiMethod
    ...
    performAPIPostRequest(method: string, body: any): Observable<any> {
        return this.getSession().concatMap(session => {
            ...
            const url = `${VKConsts.apiUrl}${method}`;
    ...
    performSingleAPIRequest(method: string, parameters: any): Observable<any> {
        ...
        const result = this.getSession().concatMap(session => {
            if (!session) {
                console.log('session is null, not authorized');
                this.authorized = false;
                return Observable.throw({
    ...
    private createAjaxPostRequest(url: string, requestData: FormData): Observable<any> {
        return Observable.create((observer: any) => {
                console.log('create ajax', url, requestData);
                $.ajax({
                    url: url,
                    type: 'POST',
                    data: requestData,
    ...
    private createUrl(session: SessionInfo, requests: any[]) {
        ...
        const requestsBatch = requests.map(r => {
            const params = Object.assign({}, r.parameters);
            const url = `API.${r.method}(${JSON.stringify(params)})`;
            return { id: r.id, url };
        }).map(r => `"${r.id}": ${r.url}`).join(', ');
        // tslint:disable-next-line:max-line-length
        const url = `${VKConsts.apiUrl}execute?access_token=${session.accessToken}&v=${VKConsts.apiVersion}&lang=${this.lang}&code=return {${requestsBatch}};`;
