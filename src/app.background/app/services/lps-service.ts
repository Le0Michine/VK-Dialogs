...
export class LPSService {
    ...
    private getLpsApiMethod = 'messages.getLongPollServer';
    ...
    private startLongPollRequest(server: LongPollServerInfo) {
        ...
        const uri = `https://${server.server}?act=a_check&key=${server.key}&ts=${server.ts}&wait=25&mode=2`;
        console.log(`long poll uri: ${uri}`);
        return Observable.merge(
            // this.http.get(uri).timeout(35000, new Error("35s timeout occured")).map(response => response.json()),
            this.http.get(uri).retryWhen(e => e.delay(1000)).timeout(35000).map(response => response.json()),
