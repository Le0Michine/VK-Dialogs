...
export function sendMessageSuccess(id: number): Action {
    return { type: InputMessageActions.SEND_MESSAGE_SUCCESS, payload: { peerId: id } };
}

export function sendMessageFail(id: number): Action {
    return { type: InputMessageActions.SEND_MESSAGE_FAIL, payload: { peerId: id } };
}

export function restoreInputMessages(messages: InputMessageInfo[]): Action {
    return { type: InputMessageActions.RESTORE, payload: messages.filter(m => !!m.peerId) };
}
